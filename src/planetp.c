...
#define mparallax 0.9507 /* Parallax at distance msmax from Earth */
        ...
        else
        {
            ...
            {
                strcpy(tbuf, (abs(planet_info[i].lha) < dtr(0.5)) ? "Transit" : "Up");
            }
            else
            {
                strcpy(tbuf, "Set");
...
void updatePlanet1(int i, double jd, int normal, FILE *ofile, char *obSite)
{
    char tbuf[80], obuf[90];
    char *plump=NULL;
    double d, md, mangdia, suangdia, setalt, upalt;
...
//    printf("planet name: %s\n", plnames[i]);
    phase(jd, &d, &d, &md, &mangdia, &d, &suangdia);
    jyear(jd, &yy,&mm, &dd);
    printf("%ld %04ld-%02d-%02d ",((long)jd)%1000, yy,mm,dd);
    assert(lastsaltime != -9999);
    memset(obuf, ' ', (sizeof obuf) - 1);
    obuf[(sizeof obuf) - 1] = 0;

    sprintf(tbuf, "%s", plnames[i]);
    memcpy(obuf, tbuf, strlen(tbuf));
    {
        //double m = fixangle(planet_info[i].ra);
        sprintf(tbuf, "%03.1lf %03.1lf", planet_info[i].ra, planet_info[i].hlong);
//        sprintf(tbuf, "%2dh %2dm %2ds", (int)m, ((int)(m * 60)) % 60,
//                (int)fmod(m * 3600, 60.0));
    }
    Set(22);
